...
After publication, test the registry version in a fresh prefix, remove the pending-release note in the README and update the server's `/after-effects` setup instructions. Only then describe the npm installation as available. Use a new version for subsequent releases and update pinned setup examples together.
