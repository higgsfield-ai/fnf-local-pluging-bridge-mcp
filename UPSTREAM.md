...
FNF changes add offline AE skill delivery, a bundled skill corpus, local setup diagnostics, installation support and validation. `skills/manifest.json` identifies the source repository, skill path and SHA-256 of every served document. Source instructions originate in the private FNF bridge project and are maintained under `skills/` in this repository (originally `fnf-creative-apps-skills`). They are not covered by the upstream runtime's MIT grant; no additional redistribution license is granted here.
